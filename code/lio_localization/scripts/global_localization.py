...
def crop_global_map_in_FOV(global_map, pose_estimation, cur_odom):
    ...
    if FOV > 3.14:
        # 环状lidar 仅过滤距离
        indices = np.where(
            (global_map_in_base_link[:, 0] > -(FOV_FAR+FOV_FAR_offset)) &
            (global_map_in_base_link[:, 0] < (FOV_FAR+FOV_FAR_offset)) &
            (global_map_in_base_link[:, 1] > -(FOV_FAR+FOV_FAR_offset)) &
            (global_map_in_base_link[:, 1] < (FOV_FAR+FOV_FAR_offset)) &
        ...
        indices = np.where(
            (global_map_in_base_link[:, 0] > -FOV_FAR_edge) &
            (global_map_in_base_link[:, 0] < (FOV_FAR+FOV_FAR_offset)) &
            (global_map_in_base_link[:, 1] > -(FOV_FAR+FOV_FAR_offset)) &
            (global_map_in_base_link[:, 1] < (FOV_FAR+FOV_FAR_offset)) &
            (np.abs(np.arctan2(global_map_in_base_link[:, 1], global_map_in_base_link[:, 0])) < FOV / 2.0)
...
def crop_scan_in_FOV(cur_scan, cur_odom):
    ...
    # 把地图转换到lidar系下
    # scan虽然是转换到不同坐标系下了
    # 但是每个点的索引都是一样的 所以可以在base_link系下滤波 用索引直接获取odom下的点云
    ...
    if FOV > 3.14:
        # 环状lidar 仅过滤距离
        indices = np.where(
            (cur_scan_in_base_link[:, 0] > -FOV_FAR) &
            (cur_scan_in_base_link[:, 0] < FOV_FAR) &
            (cur_scan_in_base_link[:, 1] > -FOV_FAR) &
            (cur_scan_in_base_link[:, 1] < FOV_FAR) &
            (np.abs(np.arctan2(cur_scan_in_base_link[:, 1], cur_scan_in_base_link[:, 0])) < FOV / 2.0)
        )
    else:
        # 非环状lidar 保前视范围
        # FOV_FAR>x>0 且角度小于FOV
        indices = np.where(
            (cur_scan_in_base_link[:, 0] > -FOV_FAR_edge) &
            (cur_scan_in_base_link[:, 0] < FOV_FAR) &
            (cur_scan_in_base_link[:, 1] > -FOV_FAR) &
            (cur_scan_in_base_link[:, 1] < FOV_FAR) &
            (np.abs(np.arctan2(cur_scan_in_base_link[:, 1], cur_scan_in_base_link[:, 0])) < FOV / 2.0)
    ...
    # 用索引直接获取滤波后的odom下的点云
    cur_scan_in_FOV = o3d.geometry.PointCloud()
    cur_scan_in_FOV.points = o3d.utility.Vector3dVector(np.squeeze(cur_scan_in_odom[indices, :3]))

    header = cur_odom.header
    header.frame_id = 'camera_init'
    publish_point_cloud(pub_pc_in_map, header, np.array(cur_scan_in_FOV.points))

    return cur_scan_in_FOV
...
def global_localization(pose_estimation):
    ...
    scan_in_FOV = crop_scan_in_FOV(scan_tobe_mapped, cur_odom)

    global_map_in_FOV = crop_global_map_in_FOV(global_map, pose_estimation, cur_odom)

    # 粗配准
    transformation, _fit = registration_at_scale(scan_in_FOV, global_map_in_FOV, initial=pose_estimation, scale=5)
    rospy.loginfo('fitness score:{}'.format(_fit))

    # 精配准
    transformation, fitness = registration_at_scale(scan_in_FOV, global_map_in_FOV, initial=transformation,scale=0.5)
    ...
    t_err = np.linalg.norm(T_map_to_odom[:3,3] - transformation[:3,3])
    rospy.loginfo('t_err: {}'.format(t_err))
    ...
    if not initialized:
        ...
        if fitness > LOCALIZATION_TH_max :
            # T_map_to_odom = np.matmul(transformation, pose_estimation)
            T_map_to_odom = transformation
            ...
            pub_map_to_odom.publish(map_to_odom)
            return True
        elif fitness > LOCALIZATION_TH and t_err < 0.5:
            T_map_to_odom = transformation
...
def cb_save_cur_scan(pc_msg):
    ...
    #     print(f"Name: {f.name} | Type: {f.datatype} | Offset: {f.offset}")
...
if __name__ == '__main__':
    ...
    LOCALIZATION_TH_max = 0.99
    LOCALIZATION_TH_init = 0.9
    ...
    while not initialized:
        ...
        trans_odom_to_base_link = pose_to_mat(cur_odom)
        trans_odom_to_base_link_inverse = inverse_se3(trans_odom_to_base_link)
        trans_map_to_odom = np.matmul(initial_pose, trans_odom_to_base_link_inverse)
        if cur_scan:
            initialized = global_localization(trans_map_to_odom)
        else:
